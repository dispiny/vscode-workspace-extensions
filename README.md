...
- **넓게 보기 (에디터 탭)**: 사이드바 헤더의 **확장해서 열기**(또는 명령 **Aurora: Open in Editor**) →
  같은 UI를 에디터 영역 전체 폭의 탭으로 엽니다. 탭을 닫으면 원래 레이아웃으로 복귀하며,
  사이드바와 상태(워크스페이스 목록 등)가 실시간 동기화됩니다. (VS Code는 사이드바 폭 자체를
  확장이 조절하는 API를 제공하지 않아, 넓게 보려면 에디터 탭으로 여는 방식을 씁니다.)
